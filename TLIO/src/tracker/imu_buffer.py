...
class ImuBuffer:
    ...
    def get_data_from_to(self, t_begin_us: int, t_us_end: int, step:int):
        ...
        net_acc = self.net_acc[begin_idx-step : end_idx + 1, :]
        net_gyr = self.net_gyr[begin_idx-step : end_idx + 1, :]
        net_t_us = self.net_t_us[begin_idx-step : end_idx + 1]
        return net_acc, net_gyr, net_t_us
    ...
    def throw_data_before(self, t_begin_us: int):
        ...
        assert isinstance(t_begin_us, int)
        begin_idx = np.where(self.net_t_us == t_begin_us)[0][0]
        self.net_acc = self.net_acc[begin_idx-self.extra:, :]
        self.net_gyr = self.net_gyr[begin_idx-self.extra:, :]
        self.net_t_us = self.net_t_us[begin_idx-self.extra:]
