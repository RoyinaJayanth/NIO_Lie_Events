...
class ImuTrackerRunner:
    ...
    def __init__(self, args, dataset):
        ...
        ## IMU Tracker if fixed time interval
        self.tracker = ImuTracker(
            model_path=args.model_path,
            model_param_path=args.model_param_path,
            net_samples=args.net_samples,
            imu_freq_net = args.imu_freq_net,
            update_freq=args.update_freq,
            filter_tuning_cfg=filter_tuning,
            imu_calib=imu_calib,  
            initial_velocity_debug = args.initial_velocity_debug,
            event_based_input = args.event_based_input,
            base_event_stack = args.base_event_stack,
            geodesic_event = args.geodesic_event,
            integration_imu_frame = args.integration_imu_frame,
            rot_component_weight = args.rot_component_weight,
            se3_events=args.se3_events, 
            contrast_threshold=args.contrast_threshold, 
            extra = args.extra_imu_buffer_length,
            polarity_input = args.polarity_input,
            only_polarity_input = args.only_polarity_input,
            vio_initialise_preint = args.vio_initialise_preint,
            vio_preint = args.vio_preint
            #force_cpu=True,
        )
        ...
        self.propagation_step_size = args.imu_base_freq/args.propagation_freq
    ...
    def add_data_to_be_logged(self, ts, acc, gyr, with_update):
        # filter data logger
        R, v, p, ba, bg = self.tracker.filter.get_evolving_state(int(ts * 1e6))
        ba = ba
        bg = bg
        ...
        if not with_update:
            ...
            meas_sigma *= np.nan
            inno_sigma *= np.nan
            last_ev_ts = np.array([0]).reshape(1,1)
            
        else:
            last_ev_ts = np.array([self.tracker.filter.state.si_timestamps_us[-1]]).reshape(1,1) *1e-6
    ...
    def run_tracker(self, args):
        ...
        if args.use_vio_meas or args.initial_velocity_debug or args.vio_initialise_preint or args.vio_preint:
            self.tracker.debug_callback_get_meas = lambda t0, t1, init_vel=False, vio_preint=False, net_tus = None: self.input.get_meas_from_vio(
                t0, t1, init_vel=init_vel, vio_preint=vio_preint, net_tus = net_tus
            )

        # Loop through the entire dataset and feed the data to the imu tracker
        n_data = self.input.dataset_size
        for i in tqdm(range(0,n_data,int(self.propagation_step_size))):
            # print(i)
        ...
        # if self.propagation_step_size!=1:
        #     ts, acc_raw, gyr_raw = self.input.get_datai(n_data-1)
        #     t_us = int(ts * 1e6)
        #     did_update = self.tracker.on_imu_measurement(t_us, gyr_raw, acc_raw)
        #     self.add_data_to_be_logged(
        #         ts,
