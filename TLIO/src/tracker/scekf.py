...
class ImuMSCKF:
    ...
    def get_past_state(self, t_us):
        ...
        vel = (((self.state.si_ps[state_idx+1] - self.state.si_ps[state_idx])/((self.state.si_timestamps_us[state_idx+1] - self.state.si_timestamps_us[state_idx])*1e-6))+((self.state.si_ps[state_idx+2] - self.state.si_ps[state_idx+1])/((self.state.si_timestamps_us[state_idx+2] - self.state.si_timestamps_us[state_idx+1])*1e-6)))/2
        vel_ps = self.state.si_vel[state_idx]
        return R, p, vel, vel_ps
    
    def get_evolving_state(self, ts_us):
    ...
    def marginalize(self, cut_idx):
        ...
        self.state.si_vel = self.state.si_vel[cut_idx + 1 :]
