...
class GlobSpeedSequence_test(CompiledSequence):
    """
...
class DenseSequenceDataset_test(Dataset):
...
class StridedSequenceDataset_test(Dataset):
    def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=10, window_size=200,
                 random_shift=0, transform=None, mode = 'train', contrast_threshold = 0.1, 
                 add_vel_perturb_range = 0.5, polarity_noise_range = 0.5, **kwargs):
        super(StridedSequenceDataset_test, self).__init__()
        self.feature_dim = seq_type.feature_dim
        self.target_dim = seq_type.target_dim
        self.aux_dim = seq_type.aux_dim
        self.window_size = window_size
        ...
        for i in range(len(data_list)):
            ...
            self.orientations.append(aux[i][:, 1:5])
            self.gt_pos.append(aux[i][:, -3:])
            self.index_map += [[i,j] for j in range(200, self.targets[i].shape[0], step_size)]#step_size
    ...
    def __getitem__(self, item):
        seq_id, frame_id = self.index_map[item][0], self.index_map[item][1]
        feat = self.features[seq_id][frame_id:frame_id + self.window_size].astype('float64')
        targ = self.targets[seq_id][frame_id]
        ...
        return feat.T, \
               ...
               seq_id, frame_id, \
                self.orientations[seq_id][frame_id:frame_id + self.window_size].astype('float64'), \
                    self.targets[seq_id][frame_id-200].astype('float64'), \
                        self.gt_pos[seq_id][frame_id:frame_id+2].reshape((-1,3)).astype('float64'), \
                            ts
...
class SequenceToSequenceDataset_test(Dataset):
    def __init__(self, seq_type, root_dir, data_list, cache_path=None, step_size=100, window_size=400,
                 random_shift=0, transform=None, **kwargs):
        super(SequenceToSequenceDataset_test, self).__init__()
